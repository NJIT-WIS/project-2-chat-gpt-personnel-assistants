...
const runLighthouseAudit = async (pageUrl, expectedAccessibilityScore) => {
  ...
  const command = `npx lighthouse ${pageUrl} --output json --output-path ${reportPath} --only-categories=seo --chrome-flags="--headless --no-sandbox"`;
...
pages.forEach((page) => {
  test(`Page "${page.path}" should score higher than 90 on lighthouse SEO`, async ({}) => {
